...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| XSS - structured data   | `serializeJsonLd` escapes `<` to `\u003c` so content can never break out of the JSON-LD script tag (unit tested with an injection payload).                                                                          |
...
| Clickjacking            | `X-Frame-Options: DENY` + CSP `frame-ancestors 'none'` (middleware + hosting `_headers`, kept in sync by a unit test).                                                                                               |
